...
class OrObjectTypeChecker extends TsType {
  constructor(
  ...
  get elmType(): string {
    return `(Left (${this.left.elmType}) | Right (${this.right.elmType}))`
  }
...
class ObjectTypeChecker extends TsType {
  ...
  get elmType(): string {
    ...
    acc += this.fields
      .map(({ name, optional, type }) => {
        ...
        if (optional) {
          return `${name} : Maybe ${type.elmType}`
        }
        return `${name} : ${type.elmType}`
      })
      .join(', ')
    acc += ' }'
    return acc
  ...
  /**@deprecated*/
  or(that: ObjectTypeChecker): TsType {
...
class StringChecker extends TsType {
  ...
  get elmType(): string {
    return this.value.split(' ').map(capitalize).join('')
  }
...
class NumberChecker extends TsType {
...
class ArrayChecker extends TsType {
  constructor(private elementType: TsType) {
    super()
  }
  ...
  get elmType(): string {
    return `(List ${this.elementType.type})`
  }
...
function makeNativeTypeChecker(type: Type, elmType: string): TsType {
  return new (class extends TsType {
    type: string = type
...
const nativeTypeCheckers = {
  string: makeNativeTypeChecker('string', 'String'),
  number: makeNativeTypeChecker('number', 'Int'),
  Date: makeNativeTypeChecker('Date', 'String'),
  boolean: makeNativeTypeChecker('boolean', 'Bool'),
  true: makeNativeTypeChecker('true', 'True'),
  false: makeNativeTypeChecker('false', 'False'),
}

function parseOneType(s: string): ParseResult<TsType> {
  s = s.trim()
  if (s.length === 0) {
    throw new Error('empty type')
  }
  ...
  for (const typeStr of [
    ...
    if (s.startsWith(typeStr)) {
      ...
      if (!isWordChar(nextC)) {
        const type: TsType =
          nativeTypeCheckers[typeStr as keyof typeof nativeTypeCheckers]
        return {
  ...
  console.error('unknown pattern:', s)
  throw new TypeError('unknown pattern')
}

function toObjectTypeChecker(type: TsType): ObjectTypeChecker | undefined {
...
function isObjectTypeChecker(type: TsType): boolean {
  return !!toObjectTypeChecker(type)
}

class OrTypeChecker extends TsType {
  static lastErrors: Error[]

  constructor(public left: TsType, public right: TsType) {
    super()
  }
  ...
  get elmType(): string {
    let left = this.left.elmType
    let right = this.right.elmType
...
class AndTypeChecker extends TsType {
  constructor(public left: TsType, public right: TsType) {
    super()
  }
...
type LogicTerm = TsType | '|' | '&'

/**
 * & has higher binding order than |
 * */
class LogicTypeChecker extends TsType {
      ...
      for (let i = 0; i < xs.length; i++) {
        ...
        if (curr === op) {
          ...
          if (!left || typeof left === 'string') {
            throw new Error(`missing left-hand-side term for logical op: ` + op)
          }
          const right = xs[i + 1]
          i++
...
class BracketTypeChecker<T extends TsType = TsType> extends TsType {
...
export function parseType(s: string): ParseResult<TsType> {
  const originalS = s
