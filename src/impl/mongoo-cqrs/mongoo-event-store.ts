...
export class MongooEventStoreImpl<T> implements EventStore<T> {
  ...
  constructor(@InjectModel(EventDBObject) public readonly model: ModelType<EventDBObject<T>>) {
  }
  ...
  listen(types: ConcreteTypeSelector, consumer: Consumer<Array<EventDBObject<T>>>) {
